...
function buildSearchIndex(txt:string):{name:string,lat:number,lng:number,line:string,color:string}[]{
  ...
  for(const line of metro.lines){
    for(const st of line.stations){
      ...
      if(sn.includes(q)||q.includes(sn)){results.push({name:st.name,lat:st.lat,lng:st.lon,line:line.name,color:line.color});}
    }
  }
...
export default function Map(){
  ...
  useEffect(()=>{
    ...
    map.on('click',async e=>{
      ...
      if(!rf){setRf(np);savePoint(np);setPts(getPoints());}
      else{
        ...
        if(r){
          ...
          if(map.getSource(sid)){(map.getSource(sid) as maplibregl.GeoJSONSource).setData(r.geometry);}
            ...
            map.addLayer({id:sid,type:'line',source:sid,paint:{'line-color':'#3b82f6','line-width':4}});
          }
  ...
  useEffect(()=>{
    ...
    metro.lines.forEach((line,i)=>{
      ...
      const coords=line.stations.map(s=>[s.lon,s.lat]);
  ...
  return <div style={{width:'100%',height:'100vh',position:'relative',direction:'ltr'}}>
    ...
    {(panel||sel||rr||rmode)&&<div style={{position:'absolute',top:80,right:12,width:300,maxHeight:'calc(100%-140px)',background:'rgba(15,23,42,.95)',backdropFilter:'blur(16px)',border:'1px solid #334155',borderRadius:16,padding:16,overflowY:'auto',zIndex:20}}>
      ...
      {panel==='list'&&<div>{pts.length===0?<p style={{color:'#64748b',textAlign:'center',fontSize:13}}>نقطه‌ای ذخیره نشده!</p>:pts.map(p=><div key={p.id} style={{display:'flex',alignItems:'center',gap:8,padding:'8px 10px',marginBottom:6,border:'1px solid #334155',borderRadius:8}}>
